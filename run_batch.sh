...
for pmt in ${pmt_list} ; do
	...
	if [ ${lllv} -eq 1 ] ; then
		...
		./sql_select_runs.sh pmt=${pmt} regime="ll,lv" quality recent=1 
	...
	if [ ${hlhv} -eq 1 ] ; then
		...
		./sql_select_runs.sh pmt=${pmt} regime="hl,hv" quality recent=1
	...
	if [ ${hllv} -eq 1 ] ; then
		...
		./sql_select_runs.sh pmt=${pmt} regime="hl,lv" quality recent=1
